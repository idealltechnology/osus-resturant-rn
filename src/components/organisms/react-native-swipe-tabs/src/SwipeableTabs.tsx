...
export default function SwipeableTabs(props: IProps) {
  ...
  return children && children.length ? (
    <View onTouchStart={onTouchStart} onTouchEnd={onTouchEnd} onTouchMove={onTouchMove} style={styles.window}>
      <Animated.View style={[styles.body, { width: width * Tabs.length, transform: [{ translateX: animated }] }]}>
        {Tabs.map((tab: any, index: number) => (
          <View key={index} style={styles.tab}>
            <View style={styles.tabContent}>{tab}</View>
          </View>
        ))}
      </Animated.View>
      {!!labels && (
...
class TabStateWrapper extends React.Component<{ TabBar: any } & ITabProps, { selectedIndex: number }> {
  ...
  componentDidUpdate(prevProps: Readonly<{ TabBar: any } & ITabProps>): void {
    if (prevProps.selectedIndex != this.props.selectedIndex) this.setState({ selectedIndex: this.props.selectedIndex });
  }
}
